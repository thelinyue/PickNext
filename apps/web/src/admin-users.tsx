 ...
 */
export function AdminUsersPage({ onBack, notify }: { onBack(): void; notify(message: string): void }) {
  ...
  return <section className="admin-users-page">
    <header className="admin-users-header"><IconButton label="返回我的页面" onClick={onBack}><ArrowLeft /></IconButton><div><span>系统管理</span><h1>用户与权限</h1></div><button className="header-text-action" onClick={() => { setSelecting((value) => !value); setSelected(new Map()); }}>{selecting ? '完成' : '选择'}</button></header>
    <div className="admin-users-sticky">
      <div className="admin-summary"><div><strong>{summary?.total ?? '—'}</strong><span>用户</span></div><div><strong>{summary?.maintainers ?? '—'}</strong><span>曲库管家</span></div><button onClick={() => setCreateOpen(true)}><Plus /><span>新增用户</span></button></div>
      <label className="registration-setting"><span><b>允许普通用户注册</b><small>{registration.data?.registrationOpen ? '登录页当前显示注册入口' : '仅管理员可以创建账号'}</small></span><input aria-label="允许普通用户注册" type="checkbox" checked={Boolean(registration.data?.registrationOpen)} disabled={updateRegistration.isPending} onChange={(event) => updateRegistration.mutate(event.target.checked)} /></label>
    ...
    <div className={`admin-user-list ${selecting ? 'selecting' : ''}`}>
      ...
      {rows.map((user) => <article className={`admin-user-compact ${selected.has(user.id) ? 'selected' : ''}`} key={user.id}>
        ...
        <button className="admin-user-main" onClick={() => selecting ? toggleSelected(user) : setDetailTarget(user)}><div className="user-avatar">{user.username.slice(0, 1).toUpperCase()}</div><div className="admin-user-copy"><div><strong>{user.username}</strong><span className={`role-badge ${user.role === 'admin' ? 'admin' : user.isMaintainer ? 'maintainer' : 'user'}`}>{roleLabel(user)}</span></div><p>{user.personalSongCount} 首个人歌曲 · {user.canAddSongs ? '可添加歌曲' : '禁止添加'}</p><small>{user.lastLoginAt ? `最近登录 ${relativeTime(user.lastLoginAt)}` : `从未登录 · 创建于 ${formatDate(user.createdAt)}`}</small></div><span className="row-chevron">›</span></button>
      </article>)}
