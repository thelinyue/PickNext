...
function HistorySheet({ open, onOpenChange }: { open: boolean; onOpenChange(open: boolean): void }) {
  ...
  return <Sheet open={open} onOpenChange={onOpenChange} title="点歌历史"><div className="history-stats"><div><strong>{history.data?.summary.playedTotal ?? 0}</strong><span>累计已唱</span></div><div><strong>{history.data?.summary.playedToday ?? 0}</strong><span>今天</span></div><div><strong>{history.data?.summary.favoriteArtist ?? '—'}</strong><span>常唱歌手</span></div></div><div className="scene-chips">{([['all','全部'],['week','本周'],['played','已唱']] as const).map(([value,label]) => <button className={period === value ? 'active' : ''} key={value} onClick={() => setPeriod(value)}>{label}</button>)}</div><div className="history-groups">{groups.map(([date, items]) => <details key={date}><summary><span>{date}</span><small>抽取 {items.length} 首 · 唱了 {items.filter((item) => item.status === 'played').length} 首</small></summary>{items.map((item) => <BasicSongCard key={item.id} song={{ id: item.songId, title: item.title, artist: item.artist, version: item.version, album: item.album ?? null, coverUrl: item.coverUrl ?? null, rating: item.rating }} action={<div className={`history-status ${item.status}`}>{item.status === 'played' ? '已唱' : '未唱'}{item.note && <small>{item.note}</small>}</div>} />)}</details>)}</div>{!history.isLoading && !groups.length && <EmptyState title="还没有点歌记录" description="从 Pick 开始唱第一首吧。" />}</Sheet>;
}
