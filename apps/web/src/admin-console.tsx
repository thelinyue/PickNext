...
function AdminOverview({ onOpen }: { onOpen(section: AdminSection): void }) {
  ...
  return <div className="admin-content admin-overview-content">
    <div className="admin-overview-grid"><section className="admin-panel admin-focus-panel"><div className="admin-panel-heading"><div><span className="admin-panel-kicker">优先处理</span><h2>把积压清掉</h2></div><button className="admin-inline-link" onClick={() => onOpen('tasks')}>查看全部任务 <ChevronRight size={15} /></button></div><div className="admin-focus-list"><button onClick={() => onOpen('reviews')}><span className="admin-focus-icon purple"><ClipboardCheck size={18} /></span><span><strong>{reviewTotal || '暂无'} 条审核待处理</strong><small>{reviewTotal ? '建议先处理最早提交的事项' : '新的审核会自动出现在这里'}</small></span><ChevronRight size={17} /></button><button onClick={() => onOpen('catalog')}><span className="admin-focus-icon amber"><AlertTriangle size={18} /></span><span><strong>{(value?.songsWithoutCover ?? 0) + (value?.songsWithoutLyrics ?? 0) || '暂无'} 项资料待补齐</strong><small>缺封面 {value?.songsWithoutCover ?? '—'} · 缺歌词 {value?.songsWithoutLyrics ?? '—'}</small></span><ChevronRight size={17} /></button><button onClick={() => onOpen('mtw')}><span className="admin-focus-icon cyan"><Upload size={18} /></span><span><strong>{value?.runningTasks ? '导入任务正在执行' : '开始一次 MTW 导入'}</strong><small>{value?.runningTasks ? '可在任务中心查看进度' : '扫描后再选择候选歌曲'}</small></span><ChevronRight size={17} /></button></div></section><section className="admin-panel admin-health-panel"><div className="admin-panel-heading"><div><span className="admin-panel-kicker">系统概览</span><h2>曲库状态</h2></div><CheckCircle2 className="admin-health-ok" size={19} /></div><div className="admin-health-main"><strong>{value?.songs ?? '—'}</strong><span>活跃歌曲</span></div><div className="admin-health-stats"><span><b>{value?.covers ?? '—'}</b> 已有封面</span><span><b>{value?.deletedSongs ?? '—'}</b> 已软删除</span></div><div className="admin-health-footer"><Wifi size={14} />MTW 连接状态需在系统设置中检查</div></section></div>
    <div className="admin-stat-grid admin-stat-grid-v2">{cards.map((card) => { const Icon = card.icon; return <button className={`admin-stat-card admin-stat-card-${card.tone}`} key={card.label} onClick={() => onOpen(card.target)}><span className="admin-stat-icon"><Icon size={17} /></span><strong>{card.value ?? '—'}</strong><b>{card.label}</b><small>{card.hint}</small></button>; })}</div>
